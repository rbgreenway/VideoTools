    ...
    , bEndOfDecode_(0), m_ctxLock(ctxLock)
{
    ...
    m_hFrameQueueSemaphore  = CreateSemaphore(NULL,0,INT_MAX,NULL);
}
...
FrameQueue::~FrameQueue()
{
    ...
    CloseHandle(m_hFrameQueueSemaphore);
}
...
HANDLE FrameQueue::GetFrameQueueSemaphore()
{
    return m_hFrameQueueSemaphore;
}
...
CUVIDFrameQueue::enqueue(const void * pData)
{
    ...
    do
    {
        ...
        if (nFramesInQueue_ < (int)FrameQueue::cnMaximumSize)
        {
            ...
            ReleaseSemaphore(m_hFrameQueueSemaphore,1,NULL); // increment semaphore by 1
        }
