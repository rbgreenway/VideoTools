...
int AkazeDetector::SetReferenceImage(uint8_t *pImagedata, uint32_t imageWidth, uint32_t imageHeight,
	uint32_t roiX, uint32_t roiY, uint32_t roiW, uint32_t roiH)
{
	...
	m_akaze(m_roiImage, Mat(), m_refKeypoints, m_refDescriptors);
...
double AkazeDetector::FindMatchDistance(uint8_t *pImagedata)
{
	...
	m_akaze(testRoiImage, Mat(), testKeypoints, testDescriptors);
...
double AkazeDetector::CalculateImageCorrelation(uint8_t* pImagedata) //Mat image, Mat patch)
{
	// build mats from test image
	int pad = 1;
	cv::Mat testImage = cv::Mat(m_imageHeight, m_imageWidth, CV_8UC4, pImagedata);
